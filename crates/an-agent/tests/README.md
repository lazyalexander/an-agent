...
**Hygiene:** probes self-clean temp dirs; set `AN_AGENT_PROBE_DIR` to keep a tape. Live tests are `#[ignore]`d (`react_live` / `web_search` need network; `lean_tool` needs elan+Lean 4; `bench_tool` is a bench).
